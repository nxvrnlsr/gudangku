...
const getAll = async (req, res) => {
  try {
    ...
    const [itemsResult, countResult] = await Promise.all([
      db.query(`
        SELECT i.id, i.name, i.sku, i.barcode, i.shelf_life_days,
          i.min_stock_qty, i.max_stock_qty, i.cost_price, i.is_active, i.base_unit_id,
      ...
      db.query(`SELECT COUNT(*) FROM items i ${where}`, params),
    ]);
  ...
  try {
    ...
    const [itemResult, stockResult, batchResult] = await Promise.all([
      ...
      db.query(`
        ...
        WHERE sb.item_id = $1 ORDER BY w.name
      `, [id]),
      db.query(`
    ...
    return res.status(200).json({
      status: 'success',
      data: { ...itemResult.rows[0], stock_by_warehouse: stockResult.rows, active_batches: batchResult.rows },
    });
  } catch (err) {
...
const create = async (req, res) => {
  try {
    ...
    if (!name || !sku || !base_unit_id) {
      return res.status(400).json({ status: 'error', message: req.t('items.createError') });
    }

    const result = await db.query(`
      INSERT INTO items (category_id, base_unit_id, name, sku, barcode, description, storage_notes,
    ...
    return res.status(201).json({ status: 'success', message: req.t('items.created'), data: result.rows[0] });
  } catch (err) {
    if (err.code === '23505') {
      return res.status(409).json({ status: 'error', message: req.t('items.skuTaken') });
    }
...
const update = async (req, res) => {
  try {
    ...
    return res.status(200).json({ status: 'success', message: req.t('items.updated'), data: result.rows[0] });
  } catch (err) {
    if (err.code === '23505') {
      return res.status(409).json({ status: 'error', message: req.t('items.skuTaken') });
    }
  ...
  try {
    ...
    if (totalQty > 0) {
      const msg = req.lang === 'id'
        ? `Barang tidak bisa dihapus karena masih memiliki stok aktif (${totalQty} unit). Nonaktifkan barang melalui menu Edit.`
        : `Cannot delete item: it still has active stock (${totalQty} units). Deactivate the item via the Edit menu instead.`;
    ...
    const msg = req.lang === 'id' ? `Barang "${itemName}" berhasil dihapus.` : `Item "${itemName}" deleted successfully.`;
