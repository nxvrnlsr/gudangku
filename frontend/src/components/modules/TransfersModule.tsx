...
export default function TransfersModule() {
  ...
  const handleDispatch = async (id: string) => {
    ...
    try {
      ...
      toast.success('Transfer dikirim. Status: Dalam Perjalanan');
      fetchData();
    } catch (err: unknown) {
  ...
  const handleReceive = async (id: string) => {
    ...
    try {
      ...
      toast.success('Transfer diterima. Stok gudang tujuan bertambah.');
      fetchData();
    } catch (err: unknown) {
      toast.error((err as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Gagal');
    }
  ...
  return (
    <div className="page-wrap">
      <div className="page-header">
        <h1 className="page-title">🔀 Transfer Antar Gudang</h1>
      ...
      <div style={{ padding: '12px 16px', background: 'var(--bg-elevated)', borderRadius: 'var(--r-md)', border: '1px solid var(--border)', fontSize: 'var(--text-sm)', color: 'var(--text-secondary)' }}>
        🔀 <strong style={{ color: '#EC4899' }}>2 Tahap:</strong> Kirim (stok keluar dari gudang asal) → Terima (stok masuk ke gudang tujuan)
      </div>
      ...
      <div className="panel" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div className="table-wrap" style={{ flex: 1, overflow: 'auto', border: 'none', borderRadius: 0 }}>
          {loading ? (
            ...
            <div className="empty-state"><div className="empty-icon">🔀</div><p>Belum ada transfer</p></div>
          ) : (
                  ...
                  <tr key={t.id}>
                    ...
                    <td>
                      <div className="flex gap-2">
                        {t.status === 'draft' && (
                          <button className="btn btn-primary btn-sm" onClick={() => handleDispatch(t.id)}>Kirim →</button>
                        )}
                        {t.status === 'in_transit' && (
                          <button className="btn btn-secondary btn-sm" style={{ borderColor: 'var(--success)', color: 'var(--success)' }} onClick={() => handleReceive(t.id)}>✓ Terima</button>
                        )}
                      </div>
                    </td>
                  </tr>
